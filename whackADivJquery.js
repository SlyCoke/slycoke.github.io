...
function getMole(){
	var jqxhr = $.get("/mole",function(mole){
		...
		window.setTimeout(getMole,mole.next);
...
function whack(ev){
	...
	$.ajax({
		...
		contentType:'application/json',  //default is application/x-www-form-urlencoded (fine here)
		success: $.noop //no-operation, put here for place holder (optional)
	});
	//alternatively..
	////$.post("/mole", JSON.stringify({"score":score}));
}
